...
def set_view(view_id, window, ignore_existing, single_pane):
    ...
    if not view:
        ...
        view.set_syntax_file('Packages/LaTeXOutline/outline.hidden-tmLanguage')
...
def show(window, view_id=None, ignore_existing=False, single_pane=False, other_group='', layout=1):
    ...
    if other_group:
        ...
        # get the symbol list
        unfiltered_symlist = prev_focus.get_symbols()
...
# Filters the symlist to only show sections and subsections
def filter_symlist(unfiltered_symlist):
    pattern = r'(?:Subsubs|Subs|S)ection:.*'
    filtered_symlist = [x for x in unfiltered_symlist if re.match(pattern, x[1])]
    cleaned_symlist = [(i,j.replace('\n','').replace('Subsubsection: ','    ⸱ ').replace('Subsection: ','  • ').replace('Section: ','◉ ') ) for i,j in filtered_symlist]
